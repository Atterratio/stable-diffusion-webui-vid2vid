...
from modules.devices import autocast, cpu, device, torch_gc
...
from repositories.midas.midas.transforms import NormalizeImage, PrepareForNet, Resize

from scripts.prompt_travel import process_images_after, process_images_before
...
from vid2vid_utils.img_utils import dtype, get_im, get_img, im_shift_01
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row(variant="compact").style(equal_height=True):
            working_folder = gr.Text(
                label=LABEL_WORKING_FOLDER,
                value=lambda: DEFAULT_WORKING_FOLDER,
                max_lines=1,
            )
            working_folder.change(
                fn=self._change_working_folder_input,
                inputs=working_folder,
                outputs=status_info,
                show_progress=False,
            )
            btn_open = gr.Button(value="\U0001f4c2", variant="tool")  # 📂
            btn_open.click(
                fn=self._click_working_folder_button,
                inputs=working_folder,
                outputs=status_info,
                show_progress=False,
            )
        ...
        with gr.Row():
            with gr.Tab("1: Extract frames"):
                ...
                with gr.Row(variant="compact").style(equal_height=True):
                    ...
                    video_info = gr.TextArea(
                        label=LABEL_VIDEO_INFO, max_lines=7, visible=False
                    )
                    video_file.change(
                        fn=self._change_video_file,
                        inputs=video_file,
                        outputs=[working_folder, video_info, status_info],
                        show_progress=False,
                    )

                with gr.Row(variant="compact").style(equal_height=True):
                    extract_fmt = gr.Dropdown(
                        label=LABEL_EXTRACT_FMT,
                        value=lambda: DEFAULT_EXTRACT_FMT,
                        choices=CHOICES_IMAGE_FMT,
                    )
                    extract_frame = gr.Dropdown(
                        label=LABEL_EXTRACT_FRAME,
                        value=lambda: DEFAULT_EXTRACT_FRAME,
                        choices=CHOICES_EXTRACT_FRAME,
                    )
                    extract_fps = gr.Slider(
                        label=LABEL_EXTRACT_FPS,
                        value=lambda: DEFAULT_EXTRACT_FPS,
                        minimum=1,
                        maximum=24,
                        step=0.1,
                    ...
                    extract_frame.change(
                        fn=lambda x: gr_show(ExtractFrame(x) == ExtractFrame.FPS),
                        inputs=extract_frame,
                        outputs=extract_fps,
                        show_progress=False,
                    )

                    btn_ffmpeg_extract = gr.Button("Extract frames!")
                    btn_ffmpeg_extract.click(
                        fn=self._frames_extract,
                ...
                gr.HTML(
                    html.escape(
                        r"=> expected to get ffprobe.json, frames\*.jpg, audio.wav"
                    )
                )
            ...
            with gr.Tab("2: Make masks & tags"):
                ...
                with gr.Row(variant="compact").style(equal_height=True):
                    midas_model = gr.Radio(
                        label=LABEL_MIDAS_MODEL,
                        value=lambda: DEFAULT_MIDAS_MODEL,
                        choices=CHOICES_MIDAS_MODEL,
                    )

                with gr.Row(variant="default").style(equal_height=True):
                    btn_frame_delta = gr.Button("Make frame delta!")
                    btn_frame_delta.click(
                        fn=self.frame_delta_extract,
                        outputs=status_info,
                        show_progress=False,
                    )
                    ...
                    btn_deepdanbooru.click(
                        fn=self.tags_extract, outputs=status_info, show_progress=False
                    )

                gr.HTML(
                    html.escape(
                        r"=> expected to get framedelta\*.png, depthmask\*.png, tags.json, tags-topk.txt"
                    )
                )
            ...
            with gr.Tab("3: Successive img2img"):
                ...
                with gr.Row(variant="compact"):
                    img2img_mode = gr.Radio(
                        label=LABEL_IMG2IMG_MODE,
                        value=lambda: DEFAULT_IMG2IMG_MODE,
                        choices=CHOICES_IMG2IMG_MODE,
                    )

                with gr.Row(variant="compact").style(equal_height=True):
                    sigma_meth = gr.Dropdown(
                        label=LABEL_SIGMA_METH,
                        value=lambda: DEFAULT_SIGMA_METH,
                        choices=CHOICES_SIGMA_METH,
                    )
                    init_noise_w = gr.Slider(
                        label=LABEL_INIT_NOISE_W,
                        value=lambda: DEFAULT_INIT_NOISE_W,
                        minimum=0.0,
                        maximum=1.0,
                        step=0.01,
                    )
                ...
                with gr.Row(
                    visible=SigmaSched(DEFAULT_SIGMA_METH) != SigmaSched.DEFAULT
                ).style(equal_height=True) as tab_sigma_sched:
                    steps = gr.Slider(
                        label=LABEL_STEPS,
                        value=lambda: DEFAULT_STEPS,
                        minimum=1,
                        maximum=150,
                        step=1,
                    )
                    denoise_w = gr.Slider(
                        label=LABEL_DENOISE_W,
                        value=lambda: DEFAULT_DENOISE_W,
                        minimum=0.0,
                        maximum=1.0,
                        step=0.01,
                    )
                    sigma_min = gr.Slider(
                        label=LABEL_SIGMA_MIN,
                        value=lambda: DEFAULT_SIGMA_MIN,
                        minimum=0.1,
                        maximum=5.0,
                        step=0.01,
                    )
                    sigma_max = gr.Slider(
                        label=LABEL_SIGMA_MAX,
                        value=lambda: DEFAULT_SIGMA_MAX,
                        minimum=0.1,
                        maximum=5.0,
                        step=0.01,
                    )
                ...
                sigma_meth.change(
                    fn=lambda x: gr_show(SigmaSched(x) != SigmaSched.DEFAULT),
                    inputs=sigma_meth,
                    outputs=tab_sigma_sched,
                    show_progress=False,
                )

                with gr.Group() as tab_extras:
                    with gr.Row(variant="compact").style(equal_height=True):
                        fdc_methd = gr.Dropdown(
                            label=LABEL_FDC_METH,
                            value=lambda: DEFAULT_FDC_METH,
                            choices=CHOICES_FDC_METH,
                        )
                        delta_mask = gr.Dropdown(
                            label=LABEL_DELTA_MASK,
                            value=lambda: DEFAULT_DELTA_MASK,
                            choices=CHOICES_MASK,
                        )
                        spatial_mask = gr.Dropdown(
                            label=LABEL_SPATIAL_MASK,
                            value=lambda: DEFAULT_SPATIAL_MASK,
                            choices=CHOICES_MASK,
                        )
                    with gr.Row(variant="compact").style(
                        equal_height=True
                    ) as tab_params:
                        motion_highext = gr.Slider(
                            label=LABEL_MOTION_HIGHEXT,
                            value=lambda: DEFAULT_MOTION_HIGHEXT,
                            minimum=1,
                            maximum=15,
                            step=2,
                        )
                        motion_lowcut = gr.Slider(
                            label=LABEL_MOTION_LOWCUT,
                            value=lambda: DEFAULT_MOTION_LOWCUT,
                            minimum=0,
                            maximum=255,
                            step=8,
                        )
                        depth_lowcut = gr.Slider(
                            label=LABEL_DEPTH_LOWCUT,
                            value=lambda: DEFAULT_DEPTH_LOWCUT,
                            minimum=0,
                            maximum=255,
                            step=8,
                            interactive=MaskType(DEFAULT_SPATIAL_MASK)
                            == MaskType.DEPTH,
                        )
                    ...
                    def switch_params(delta_mask, spatial_mask):
                        ...
                        show_tab = (
                            spatial_mask != MaskType.NONE or delta_mask != MaskType.NONE
                        )
                        act_motion = (
                            spatial_mask == MaskType.MOTION
                            or delta_mask == MaskType.MOTION
                        )
                        act_depth = (
                            spatial_mask == MaskType.DEPTH
                            or delta_mask == MaskType.DEPTH
                        )
                        return [
                            gr_show(show_tab),
                    ...
                    delta_mask.change(
                        fn=switch_params,
                        inputs=[delta_mask, spatial_mask],
                        outputs=[
                            tab_params,
                            motion_highext,
                            motion_lowcut,
                ...
                img2img_mode.change(
                    fn=lambda x: gr_show(Img2ImgMode(x) == Img2ImgMode.BATCH),
                    inputs=img2img_mode,
                    outputs=tab_extras,
                    show_progress=False,
                )
                ...
                gr.HTML(
                    html.escape(r"=> expected to get img2img\*.png, motionmask\*.png")
                )
            ...
            with gr.Tab("4: Upscale & interpolate"):
                ...
                with gr.Row(variant="compact").style(equal_height=True):
                    resr_model = gr.Dropdown(
                        label=LABEL_RESR_MODEL,
                        value=lambda: DEFAULT_RESR_MODEL,
                        choices=CHOICES_RESR_MODEL,
                    )
                    btn_resr = gr.Button("Launch super-resolution!")
                    btn_resr.click(
                        fn=self.resr_generation,
                        inputs=resr_model,
                        outputs=status_info,
                        show_progress=False,
                    )
                ...
                with gr.Row(variant="compact").style(equal_height=True):
                    rife_model = gr.Dropdown(
                        label=LABEL_RIFE_MODEL,
                        value=lambda: DEFAULT_RIFE_MODEL,
                        choices=CHOICES_RIFE_MODEL,
                    )
                    rife_ratio = gr.Slider(
                        label=LABEL_RIFE_RATIO,
                        value=lambda: DEFAULT_RIFE_RATIO,
                        minimum=0.5,
                        maximum=4.0,
                        step=0.1,
                    )
                    ...
                    btn_rife.click(
                        fn=self.rife_generation,
                        inputs=[rife_model, rife_ratio, extract_fmt],
                        outputs=status_info,
                        show_progress=False,
                    )
            ...
            with gr.Tab("5: Export"):
                ...
                with gr.Row(variant="compact").style(equal_height=True):
                    export_fmt = gr.Dropdown(
                        label=LABEL_EXPORT_FMT,
                        value=lambda: DEFAULT_EXPORT_FMT,
                        choices=CHOICES_VIDEO_FMT,
                    )
                    frame_src = gr.Dropdown(
                        label=LABEL_FRAME_SRC,
                        value=lambda: DEFAULT_FRAME_SRC,
                        choices=CHOICES_FRAME_SRC,
                    )

                    btn_ffmpeg_compose = gr.Button("Export!")
                    btn_ffmpeg_compose.click(
                        fn=self.ffmpeg_export,
                        inputs=[
                            export_fmt,
                            frame_src,
                            extract_fmt,
        ...
        with gr.Row(variant="compact").style(equal_height=True):
            allow_overwrite = gr.Checkbox(
                label=LABEL_ALLOW_OVERWRITE, value=lambda: DEFAULT_ALLOW_OVERWRITE
            )
            allow_overwrite.change(
                fn=self._change_allow_overwrite, inputs=allow_overwrite
            )

            process_audio = gr.Checkbox(
                label=LABEL_PROCESS_AUDIO, value=lambda: DEFAULT_PROCESS_AUDIO
            )
            process_audio.change(fn=self._change_process_audio, inputs=process_audio)

            btn_interrut = gr.Button("Interrupt!", variant="primary")
            btn_interrut.click(
                fn=self._click_interrupt, outputs=status_info, show_progress=False
            )
    ...
    def run(
        ...
        if img2img_mode == Img2ImgMode.BATCH:
            ...
            if "check cache exists":
                out_dp = self.workspace.images_cache
                if out_dp.exists():
            ...
            if "check required materials exist":
                ...
                if not frames_dp.exists():
                    return Processed(
                        p, [], p.seed, f"frames folder not found: {frames_dp}"
                    )
                ...
                if require_delta:
                    if not delta_dp.exists():
                        return Processed(
                            p, [], p.seed, f"framedelta folder not found: {delta_dp}"
                        )
                ...
                if require_depth:
                    if not depth_dp.exists():
                        return Processed(
                            p, [], p.seed, f"mask folder not found: {depth_dp}"
                        )
        ...
        if sigma_meth != SigmaSched.DEFAULT:
            ...
            p.sampler_noise_scheduler_override = lambda step: sigma_fn(step).to(
                p.sd_model.device
            )
        ...
        def cfg_denoiser_hijack(param: CFGDenoiserParams):
            if not "show real sigma":
                print(
                    f">> [{param.sampling_step + 1}/{param.total_sampling_steps}] sigma: {param.sigma[-1].item()}"
                )

        env = globals()
        runner = (
            self.run_batch_img2img
            if img2img_mode == Img2ImgMode.BATCH
            else self.run_img2img
        )
    ...
    def _task(fn):
        def wrapper(self, *args, **kwargs):
            task_name = fn.__name__.replace("_", " ")
            ...
            if self.workspace is None:
                ...
                code = RetCode.ERROR
                info = f'task "{self.task_name}" is stilling running!'
                ts = None
            else:
                self.task_name = task_name
                print(f">> run task {self.task_name}")
                state.interrupted = False
                _ts = time()
                code, info = fn(self, *args, **kwargs)
                ts = time() - _ts
                self.task_name = None
    ...
    def _task_ignore_str(self) -> str:
        return f'task "{self.task_name}" ignored due to cached already exists :)'
    ...
    def _change_working_folder_input(self, working_folder: str) -> GradioRequest:
        ...
        if working_folder.is_dir():
            ...
            return gr_update_status(
                f"invalid folder path: {working_folder}", code=RetCode.WARN
            )
    ...
    def _click_working_folder_button(self, working_folder: str) -> GradioRequest:
        if Path(working_folder).is_dir():
            ...
            return gr_update_status(
                f"invalid folder path: {working_folder!r}", code=RetCode.ERROR
            )
    ...
    def _change_video_file(self, video_file):
        ...
        if video_file is None:
            ...
            # open workspace
            try:
                self.workspace = Workspace(video_file, self.cache_folder)
                ...
                return [
                    gr.Text.update(
                        label=LABEL_WORKSPACE_FOLDER,
                        value=self.workspace.workspace,
                        interactive=False,
                    ),
    ...
    def _frames_extract(
        ...
        if self.workspace.frames_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            shutil.rmtree(self.workspace.frames_cache)

        self.workspace.frames_cache.mkdir()

        self.workspace.audio_cache.unlink(missing_ok=True)
        ...
        try:
            ...
            if extract_frame == ExtractFrame.FPS:
                cmd = f'"{FFMPEG_BIN}" -i "{video_file.name}" -an -sn -f image2 -q:v 2 -r {extract_fps} "{self.workspace.frames_cache}{os.sep}%05d.{extract_fmt}"'
            elif extract_frame == ExtractFrame.IPB:
                cmd = f'"{FFMPEG_BIN}" -i "{video_file.name}" -an -sn -f image2 -q:v 2 -fps_mode vfr "{self.workspace.frames_cache}{os.sep}%05d.{extract_fmt}"'
            else:  # I/P/B
                cmd = f'"{FFMPEG_BIN}" -i "{video_file.name}" -an -sn -f image2 -q:v 2 -fps_mode vfr -vf "select=eq(pict_type\,{extract_frame.name})" "{self.workspace.frames_cache}{os.sep}%05d.{extract_fmt}"'
            self._run_subprocess(cmd)

            has_audio = "no"
            if self.process_audio:
                for stream in self.workspace.ffprob_info["streams"]:
                    if stream["codec_type"] == "audio":
                        self._run_subprocess(
                            f'"{FFMPEG_BIN}" -i "{video_file.name}" -vn -sn "{self.workspace.audio_cache}"'
                        )
                        has_audio = "yes"
                        break
            ...
            return (
                RetCode.INFO,
                f"frames: {get_folder_file_count(self.workspace.frames_cache)}, audio: {has_audio}",
            )
    ...
    def frame_delta_extract(self) -> TaskResponse:
        if not self.workspace.frames_cache.exists():
            return (
                RetCode.ERROR,
                f"frames folder not found: {self.workspace.frames_cache}",
            )

        if self.workspace.delta_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            shutil.rmtree(self.workspace.delta_cache)

        self.workspace.delta_cache.mkdir()

        try:
            fps = list(self.workspace.frames_cache.iterdir())
            ...
            for fp in tqdm(fps[1:]):
                ...
                img = im_to_img(im)
                img.save(self.workspace.delta_cache / f"{fp.stem}.png")

            return (
                RetCode.INFO,
                f"framedelta: {get_folder_file_count(self.workspace.delta_cache)}",
            )
        except KeyboardInterrupt:
            return RetCode.WARN, "interrupted by Ctrl+C"
        except:
    ...
    def frame_depth_extract(self, midas_model) -> TaskResponse:
        ...
        if self.workspace.depth_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            shutil.rmtree(self.workspace.depth_cache)

        self.workspace.depth_cache.mkdir()
        ...
        try:
            ...
            if midas_model == MidasModel.DPT_LARGE:
                ...
                model = DPTDepthModel(
                    path=model_path, backbone="vitb_rn50_384", non_negative=True
                )
                net_w, net_h = 384, 384
                resize_mode = "minimal"
                normalization = NormalizeImage(
            ...
            with torch.no_grad(), autocast():
                for fn in tqdm(
                    list(self.workspace.frames_cache.iterdir())
                ):  # TODO: make batch for speedup
                    if state.interrupted:
                        break
                    ...
                    X = (
                        torch.from_numpy(X_np).to(device).unsqueeze(0)
                    )  # [B=1, C, maxH, maxW], float32
                    if device == cuda:
                    ...
                    depth = (
                        depth.squeeze().cpu().numpy().astype(dtype)
                    )  # [H, W], float32
                    ...
                    img.save(self.workspace.depth_cache / f"{Path(fn).stem}.png")

            return (
                RetCode.INFO,
                f"depth_masks: {get_folder_file_count(self.workspace.depth_cache)}",
            )
        except KeyboardInterrupt:
            return RetCode.WARN, "interrupted by Ctrl+C"
        except Exception as e:
    ...
    def tags_extract(self, topk=32) -> TaskResponse:
        ...
        if self.workspace.tags_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            self.workspace.tags_cache.unlink()

        try:
            tags: Dict[str, str] = {}
            deepbooru_model.start()
            for fp in tqdm(sorted(self.workspace.frames_cache.iterdir())):
            ...
            tags_topk = ", ".join(
                [t for t, c in Counter(tags_flatten).most_common(topk)]
            )

            with self.workspace.tags_cache.open("w") as fh:
                json.dump(tags, fh, indent=2, ensure_ascii=False)

            with self.workspace.tags_topk_cache.open("w") as fh:
                fh.write(tags_topk)

            return (
                RetCode.INFO,
                f"prompts: {len(tags)}, top-{topk} freq tags: {tags_topk}",
            )
        except KeyboardInterrupt:
            return RetCode.WARN, "interrupted by Ctrl+C"
        except Exception as e:
    ...
    def resr_generation(self, resr_model: str) -> TaskResponse:
        if not self.workspace.images_cache.exists():
            return (
                RetCode.ERROR,
                f"img2img folder not found: {self.workspace.images_cache}",
            )

        if self.workspace.resr_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            shutil.rmtree(self.workspace.resr_cache)

        self.workspace.resr_cache.mkdir()
        ...
        try:
            ...
            self._run_subprocess(
                f'"{RESR_BIN}" -v -s {resr_ratio} -n {resr_model} -i "{self.workspace.images_cache}" -o "{self.workspace.resr_cache}"'
            )

            return (
                RetCode.INFO,
                f"upscaled: {get_folder_file_count(self.workspace.resr_cache)}",
            )
        except KeyboardInterrupt:
            return RetCode.WARN, "interrupted by Ctrl+C"
        except Exception as e:
    ...
    @_task
    def rife_generation(
        self, rife_model: str, rife_ratio: float, extract_fmt: str
    ) -> TaskResponse:
        if not self.workspace.resr_cache.exists():
            return RetCode.ERROR, f"resr folder not found: {self.workspace.resr_cache}"

        if self.workspace.rife_cache.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()

            shutil.rmtree(self.workspace.rife_cache)

        self.workspace.rife_cache.mkdir()

        try:
            n_interp = int(
                get_folder_file_count(self.workspace.resr_cache) * rife_ratio
            )
            self._run_subprocess(
                f'"{RIFE_BIN}" -v -n {n_interp} -m {rife_model} -f %05d.{extract_fmt} -i "{self.workspace.resr_cache}" -o "{self.workspace.rife_cache}"'
            )

            return (
                RetCode.INFO,
                f"interpolated: {get_folder_file_count(self.workspace.rife_cache)}",
            )
        except KeyboardInterrupt:
            return RetCode.WARN, "interrupted by Ctrl+C"
        except Exception as e:
    ...
    def ffmpeg_export(
        ...
        in_dp = self.workspace.workspace / frame_src
        if not in_dp.exists():
            return RetCode.ERROR, f"src folder not found: {in_dp}"

        audio_opts = ""
        in_fp = self.workspace.audio_cache
        if self.process_audio and in_fp.exists():
            audio_opts += f' -i "{in_fp}"'

        out_fp = self.workspace.workspace / f"{WS_SYNTH}-{frame_src}.{export_fmt}"
        if out_fp.exists():
            if not self.allow_overwrite:
                return RetCode.WARN, self._task_ignore_str()
        ...
        def get_real_fps() -> float:
            ...
            if real_fps is None:  # if video fps available
                try:
                    n_inits = get_folder_file_count(
                        self.workspace.workspace / WS_FRAMES
                    )
                    for stream in self.workspace.ffprob_info["streams"]:
                        if stream["codec_type"] == "video":
                            real_fps = (
                                float(stream["avg_frame_rate"]) * n_frames / n_inits
                            )
                            break
                except:
                    pass
        ...
        try:
            try:
                self._run_subprocess(
                    f'"{FFMPEG_BIN}"{audio_opts} -framerate {get_real_fps()} -i "{in_dp}{os.sep}%05d{get_ext()}" -crf 20 -c:v libx264 -pix_fmt yuv420p "{out_fp}"'
                )
            except:
                self._run_subprocess(
                    f'"{FFMPEG_BIN}"{audio_opts} -framerate {get_real_fps()} -i "{in_dp}{os.sep}%05d{get_ext()}" "{out_fp}"'
                )
    ...
    def _click_interrupt(self) -> GradioRequest:
        if self.task_subprocess is not None:
            self.task_subprocess.kill()  # TODO: look like on linux close process without error.
    ...
    def run_batch_img2img(
        ...
        def image_save_hijack(param: ImageSaveParams):
            ...
            if not any([use_fdc, use_delta_mask]):
                return
            ...
            if last_frame is not None:
                ...
                if use_delta_mask:
                    if delta_mask == MaskType.MOTION:
                        mask = im_delta_to_motion(
                            tgt_d, motion_lowcut / 255.0, expand=motion_highext
                        )  # [0.0, 1.0]
                        im_to_img(mask).save(
                            motion_dp / Path(init_fns[iframe]).with_suffix(".png")
                        )  # for debug
                    ...
                    if not "debug":
                        ...
                        print(
                            f">> motion correction max: {dd.max()}, mean: {dd.mean()}"
                        )
        ...
        try:
            ...
            for i in tqdm(range(n_frames)):
                ...
                if spatial_mask == MaskType.DEPTH:
                    ...
                    spatial_mask == MaskType.MOTION and i > 0
                ):  # ignore the first frame
                    delta = get_delta(i, p.width, p.height)
                    mask = im_delta_to_motion(
                        delta, motion_lowcut / 255.0, expand=motion_highext
                    )
...
class Workspace:
    def __init__(self, video_file, working_folder: Path):
        ...
        if self.ffprob_cache.exists():
            with self.ffprob_cache.open("r") as ffp:
                ...
                self.ffprob_str = json.dumps(
                    self.ffprob_info, indent=2, ensure_ascii=False
                )
